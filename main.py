...
def test_one_epoch(args, net, test_loader):
    ...
    for src, target, rotation_ab, translation_ab in tqdm(test_loader):
        ...
        point_src.append(src.detach().cpu().numpy())
        point_tgt.append(target.detach().cpu().numpy())
    ...
    point_src = np.concatenate(point_src, axis=0)
    point_tgt = np.concatenate(point_tgt, axis=0)
    

    return total_loss * 1.0 / num_examples, rotations_ab, \
           translations_ab, rotations_ab_pred, translations_ab_pred, point_src, point_tgt
...
def test(args, net, test_loader, boardio, textio):
    with torch.no_grad():
        ...
        test_translations_ab_pred, point_src, point_tgt = test_one_epoch(args, net, test_loader)
    ...
    point_src = point_src.swapaxes(1,2)
    point_tgt = point_tgt.swapaxes(1,2)
    #print(pred_transforms.shape,point_src.shape)
    src_transformed = se3.transform(pred_transforms, point_src)#B,N,3
    point_raw = copy.deepcopy(point_tgt)
    ref_clean = point_raw
    src_clean = se3.transform(se3.concatenate(pred_transforms, se3.inverse(gt_transforms)), point_raw)
    #dist_src = np.min(square_distance(src_transformed, ref_clean))
    #dist_ref = np.min(square_distance(point_tgt, src_clean))
    #chamfer_dist = np.mean(dist_src) + np.mean(dist_ref)
...
def train(args, net, train_loader, test_loader, boardio, textio):
    ...
    for epoch in range(args.epochs):
        ...
        point_tgt = point_tgt.swapaxes(1,2)
        #print(pred_transforms.shape,point_src.shape)
        src_transformed = se3.transform(pred_transforms, point_src)
        point_raw = copy.deepcopy(point_tgt)
        ref_clean = point_raw
        src_clean = se3.transform(se3.concatenate(pred_transforms, se3.inverse(gt_transforms)), point_raw)
        #dist_src = np.min(square_distance(src_transformed, ref_clean),axis=-1)
        #dist_ref = np.min(square_distance(point_tgt, src_clean),axis=-1)
        #chamfer_dist = np.mean(np.mean(dist_src,axis=1) + np.mean(dist_ref,axis=1))

        train_rotations_ab_pred_euler = npmat2euler(train_rotations_ab_pred)
        train_eulers_ab = npmat2euler(train_rotations_ab)
        train_r_mse_ab = np.mean(np.mean((train_rotations_ab_pred_euler - train_eulers_ab) ** 2, axis=1))
        train_r_rmse_ab = np.sqrt(train_r_mse_ab)
        train_r_mae_ab = np.mean(np.mean(np.abs(train_rotations_ab_pred_euler - train_eulers_ab), axis=1))
        train_t_mse_ab = np.mean(np.mean((train_translations_ab - train_translations_ab_pred) ** 2, axis=1))
        train_t_rmse_ab = np.sqrt(train_t_mse_ab)
        train_t_mae_ab = np.mean(np.mean(np.abs(train_translations_ab - train_translations_ab_pred), axis=1))

        test_rotations_ab_pred_euler = npmat2euler(test_rotations_ab_pred)
        test_eulers_ab = npmat2euler(test_rotations_ab)
        test_r_mse_ab = np.mean(np.mean((test_rotations_ab_pred_euler - test_eulers_ab) ** 2, axis=1))
        test_r_rmse_ab = np.sqrt(test_r_mse_ab)
        test_r_mae_ab = np.mean(np.mean(np.abs(test_rotations_ab_pred_euler - test_eulers_ab), axis=1))
        test_t_mse_ab = np.mean(np.mean((test_translations_ab - test_translations_ab_pred) ** 2, axis=1))
        test_t_rmse_ab = np.sqrt(test_t_mse_ab)
        test_t_mae_ab = np.mean(np.mean(np.abs(test_translations_ab - test_translations_ab_pred), axis=1))
...
def main():
    ...
    if args.model == 'GTINet':
    
        #device_ids = [0, 1]
        net = GTINet(args).cuda()
